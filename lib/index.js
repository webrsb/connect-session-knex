...
module.exports = function (connect) {
  ...
  function KnexStore(options = {}) {
    ...
    self.schemaname = options.schemaname || null;
    ...
    self.ready = self.knex.schema
      .withSchema(self.schemaname)
      .hasTable(self.tablename)
      .then((exists) => {
        if (!exists && self.createtable) {
          return new Promise((res) => {
            isDbSupportJSON(self.knex)
              .then((isSupport) => self.knex.schema
                .withSchema(self.schemaname)
                .createTable(self.tablename, (table) => {
  ...
  KnexStore.prototype.set = function (sid, sessObject, fn) {
    ...
    if (
      ...
      return self.ready.then(() => resolve(self.knex
        .raw(getPostgresFastQuery(self.schemaname, self.tablename, self.sidfieldname), [
          sid,
          dbDate,
          sess,
        ]))
